...
async function main() {
    try {
        ...
        }
        {
            ...
            const end_key = 'marble2';
            const result = await contract.evaluateTransaction('getMarblesByRange', start_key, end_key);
            console.log(`Transaction has been evaluated, result is: ${result.toString()}`);
        }
        {
            const owner = 'tom';
            const result = await contract.evaluateTransaction('queryMarblesByOwner', owner);
            console.log(`Transaction has been evaluated, result is: ${result.toString()}`);
        }
        {
            const rich_query = '{\"selector\":{\"owner\":\"tom\"}}';
            const result = await contract.evaluateTransaction('queryMarbles', rich_query);
            console.log(`Transaction has been evaluated, result is: ${result.toString()}`);
        }
