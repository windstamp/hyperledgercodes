...
async function main() {
    try {
        ...
        }
        {
            ...
            {
                const json = {
                    marble_owner: Buffer.from('{"name":"marble2","owner":"jerry"}')
                };
                await contract.createTransaction('transferMarble').setTransient(json).submit();
                console.log('Transaction has been submitted');
            }
            {
                const name = 'marble2';
        ...
        }
        {
            ...
            {
                const json = {
                    marble_delete: Buffer.from('{"name":"marble3"}')
                };
                await contract.createTransaction('delete').setTransient(json).submit();
                console.log('Transaction has been submitted');
            }
            {
                const name = 'marble3';
